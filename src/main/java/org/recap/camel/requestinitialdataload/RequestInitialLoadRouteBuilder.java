...
public class RequestInitialLoadRouteBuilder extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from(ScsbConstants.DIRECT+ ScsbConstants.RIL_DIRECT_BARCODES_AVAILABLE_IN_LAS)
                .routeId(ScsbConstants.RIL_DIRECT_BARCODES_AVAILABLE_IN_LAS)
                .marshal().bindy(BindyType.Csv, RequestInitialLoadBarcodesInLAS.class)
                .setHeader(S3Constants.KEY, simple(requestInitialAccessionErrorFileS3Dir + "${in.header.institutionCode}" + "/${in.header.imsLocationCode}-AvailableOrExceptionInLAS_Barcodes_Failed" +"_${date:now:yyyyMMdd_HHmmss}.csv"))
                .to(ScsbConstants.SCSB_CAMEL_S3_TO_ENDPOINT);
    }
}
